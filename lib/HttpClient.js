...
class HttpClient extends EventEmitter {
  ...
  setLogger (logger) {
    for (const f of ['warn', 'log', 'debug', 'vdebug', 'vvdebug']) {
      this[f] = this.__params.logger?.[f]?.bind(this.__params.logger) ?? (() => {})
    }
  }
  ...
  #logError (error) {
    if (error.request.id !== this._errorRequestId) {
      this.log(
  ...
  #logRequest (request) {
    ...
    const parsedBody = request.parsedBody ?? request.body
    this.vdebug(
      '%s: request %s: %s %s%s', this.name, request.id,
      request.method, request.url,
      parsedBody == null ? '' : ' ' + JSON.stringify(parsedBody)
    )
    ...
    const rawBody = request.parsedBody != null ? request.body : null
    if (rawBody != null) {
      this.vvdebug(
  ...
  #logResponse (response) {
    ...
    if (response.rawBody != null) {
      this.vvdebug(
        '%s: request %d: response body: %j', this.name, response.request.id,
        response.rawBody
      )
    }
    if (response.body != null) {
      this.vdebug(
        '%s: request %d: response: %j', this.name, response.request.id,
        response.body
      )
    }
