...
class HttpError extends Error {
  constructor (message, request, statusCode, statusMessage, body) {
    super(message)
    ...
    /** @member {?*} - The parsed response body, when the server sent one.
      *
      * An error response usually carries the reason for the failure in its
      * body.  Without it, a 4xx says only that something was wrong, not what,
      * so the caller has to re-issue the request by hand to find out.
      */
    this.body = body
  }
}
            ...
            if (errorMessages.length > 0) {
              for (const errorMessage of errorMessages) {
                ...
                this.emit('error', new HttpError(
                  errorMessage, requestInfo, response.statusCode,
                  response.statusMessage, responseInfo.body
                ))
              }
              return
            }
