...
class HttpClient extends EventEmitter {
  ...
  /** Create a new instance of a client to an HTTP server.
    ...
    * the server identity.
    * @param {boolean} [params.followRedirects=true] - Automatically follow HTTP redirects.
    ...
    * @param {integer} [params.maxRedirects=5] - Maximum number of redirects to follow.
        ...
        .on('response', (response) => {
          ...
          response
            ...
            .on('end', async () => {
              ...
              if (
                this.__params.followRedirects &&
                [301, 302, 307, 308].includes(response.statusCode) &&
                response.headers.location != null
              ) {
                const redirectCount = (info._redirectCount || 0) + 1
                if (redirectCount > this.__params.maxRedirects) {
                  request.emit('error',
                    new HttpError(
                      `maximum redirects (${this.__params.maxRedirects}) exceeded`,
                ...
                const oldHttps = this.__params.https
                const newHttps = redirectUrl.protocol === 'https:'
                if (newHttps !== oldHttps) {
                  this.__params.https = newHttps
                  this._http = this.__params.https ? https : http
                  this._setUrl()
                }
                const newHost = redirectUrl.hostname +
                  (redirectUrl.port ? ':' + redirectUrl.port : '')
                if (newHost !== this.host) {
                  this.host = newHost
                }

                const redirectMethod = 
                  ([301, 302].includes(response.statusCode) && method !== 'HEAD')
                    ? 'GET'
                    : method
                const redirectBody = redirectMethod === 'GET' ? null : body
                const redirectResource = redirectUrl.pathname + redirectUrl.search
                
                try {
                  const redirectInfo = Object.assign({}, info, {
